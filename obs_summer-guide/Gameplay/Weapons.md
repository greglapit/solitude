# Weapon Themes

## Ace - Gem

* A single diamond. Point of creation, restoration. 
* Examples: Philosopher's stone
* Powers: Most creative freedom. Likely healing likely included

![[Pasted image 20251031213322.png|200]]
## 2 - Dagger(s)

* Duality, tension, harmony. Yin' and yang
* Gem exists in handle of each dagger
* Examples: Levi, Soshiro (Kaijuu), Sound Hashira
* Powers: Bleed, double attack, momentum building (attacking multiple times does more damage)

![[Dualdaggers.png|300]]

* Variants: Nunchucks. Respawning Throwing Knives
## 3 - Trident

* Illusion, seas, storm. Cycles, motion
* Gem is the three of the prongs
* Example: Poseidon
* Powers: Illusory, moving enemy, adjusting turn order, stabbing
![[Pasted image 20251031213431.png|300]]
* A Fork, an outlet
## 4 - Shield

* Cardinal directions, 4 elements, protection
* Gem is corner of each shield
* Example: Paladins
* Powers: Healing, shielding, intangability, shield bashing for stuns
* 
![[Pasted image 20251031213453.png|200]]

* Variants : Bubble
# 5 - Gauntlet

* Protection + Chaos
* Gem on each finger. Can appear as gauntlet, claw hand
* Example: Thanos, terraria, Creeping Hunger (hands eating enemies for power?)
* Powers: Double gauntlet, single, infinity stones,
![[Pasted image 20251031213533.png|300]]
## 6 - Chain
* Binding, hexes, whip
* Hexagrams
* Examples: Rider (Fate series)
* Powers: Stunning enemies, binding, self-restraining (sylas, kurupika personal vow)
* Fiddlesticks tether to all cards

![[Pasted image 20251031213553.png]]
* Variants: Spiderweb (pulling thin piece of plastic off surface of water). Tentacle. The Yoyo. Chain. String. Urumi. **String Instruments**
## 7 - Scythe
* Reaping, life-stealing, death, fates
* 7 is shaped like scythe
* Examples: Kite, Death, Grimm Reaper
* Powers: Cleaving, insta-killing, marking for death,

![[Pasted image 20251031213713.png|200]]
## 8 - Double-headed Axe

* Power, infinity, balance. cleaves both directions, Dangerous to user
* 8 shape for head of axe
* Examples: So many
* Powers: Deal damage to multiple enemies (cleave), deal damage to self

![[Pasted image 20251031213751.png|300]]
## 9 - Staves

* Completion, wisdom, reflection
* Maybe weighted end, shape like a 9 as stick and ball
* Yggdrasil, 9 worlds
* Examples: Sun Wukong
* Powers: Channeling, knowledge, counterattacking, blocking

![[Pasted image 20251031213814.png | 300]]
* Variations: Flail. Staff. Mordekaiser Weapon
## 10 - The Wheel 

* Counting, completion, rebirth,
* Examples: Mahoraga, pirate ship wheel. Giant Clock
* Powers: Countdown fights only one leaves (mahoraga summoning), enemy 

![[Pasted image 20251031213836.png|500]]